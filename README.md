...
Batch01 terrain sheets are now reference-only under `unity\Assets\Rts\Art\References\Terrain\Stage32_6ArtDirection`. Stage32.6 deletes the rejected `Batch01Imported` flat-card runtime output and maps Stage16/Stage32 player-facing set dressing to mesh/material terrain prefab assemblies under `unity\Assets\Rts\Art\Prefabs\Terrain\Stage32_6Runtime`. The old generated proxy kit remains available through `.\tools\run-stage32-terrain-kit-generator.ps1` for debug/review and fallback only.
