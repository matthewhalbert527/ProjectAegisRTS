...
- `unity`: Unity desktop board prototype, Stage 2 PC sidebar scene, Stage 3 XR board placement prototype, Stage 4 left-hand build/selection scene, Stage 5 dual-hand command scene, Stage 6 movement visualization scene, Stage 7 building power/production scene, Stage 8 art pipeline showcase scene, Stage 9 combat scene, Stage 10 economy scene, Stage 11 fog/radar/minimap scene, Stage 12 AI skirmish scene, Stage 13 map terrain pathing scene, Stage 14 feedback scene, Stage 15 performance/build-readiness scene, Stage 16/17/18/18.5/19 playable vertical slice flow, Stage 20 production proxy visuals, Stage 21 MVP visual QA, Stage 21.5 Windows display scaling, Stage 22 command controls, Stage 23 base management, Stage 24 tech/support UI, Stage 25 engineer/transport routing, Stage 26 air/naval foundations, Stage 27 skirmish playability, Stage 27.1 PC placement UX validation, Stage 28 integrated feature regression QA, Stage 29/30 visual quality/readability review scenes, Stage 31 artist handoff docs, Stage 32 terrain set-dressing review scene/catalogs, Stage32 terrain asset replacement review scene, Stage32.6 runtime/final mesh terrain review scenes, Stage33 tank source prefabs/review scene, and setup notes.
...
Stage 25 adds engineer capture, repairs, and transports:

```powershell
.\tools\run-unity-stage25-validation.ps1
.\tools\run-stage25-fast-checks.ps1
.\tools\run-stage25-medium-checks.ps1
.\tools\run-stage25-player-facing-checks.ps1 -SkipPlayerBuild
.\tools\run-stage25-checks.ps1
```

Use `run-stage25-fast-checks.ps1` while iterating on engineer capture/repair, transport load/unload, passenger snapshots, or command-bar routing. Use `run-stage25-medium-checks.ps1` before local commits; it stays flat and calls direct Stage 24, Stage 4, Stage 5, and Stage 25 validation dependencies.

Stage 26 adds airfield, aircraft, and naval foundations:

```powershell
.\tools\run-unity-stage26-validation.ps1
.\tools\run-stage26-fast-checks.ps1
.\tools\run-stage26-medium-checks.ps1
.\tools\run-stage26-player-facing-checks.ps1 -SkipPlayerBuild
.\tools\run-stage26-checks.ps1
```

Use `run-stage26-fast-checks.ps1` while iterating on helipad pads, aircraft docking, altitude snapshots, water/naval passability, or vertical-slice air assets. Use `run-stage26-medium-checks.ps1` before local commits; it stays flat and calls direct Stage 25, Stage 4, Stage 5, and Stage 26 validation dependencies.

Stage 27 adds skirmish playability and AI pressure polish:

```powershell
.\tools\run-unity-stage27-validation.ps1
.\tools\run-stage27-fast-checks.ps1
.\tools\run-stage27-medium-checks.ps1
.\tools\run-stage27-player-facing-checks.ps1 -SkipPlayerBuild
.\tools\run-stage27-checks.ps1
```

Use `run-stage27-fast-checks.ps1` while iterating on AI difficulty, attack-wave timing, restart controls, or the skirmish status HUD. Use `run-stage27-medium-checks.ps1` before local commits; it stays flat and calls direct Stage 26, Stage 4, Stage 5, and Stage 27 validation dependencies.

Stage 27.1 fixes the PC building placement overlay regression:
...
Use `run-stage27-1-fast-checks.ps1` while iterating on PCDesktop building placement, placement-panel visibility, or pause/cancel behavior. Use `run-stage27-1-medium-checks.ps1` before local commits; it stays flat and calls direct Stage 27 player-facing validation, Stage 4/5 hand-control validation, and Stage 27.1 validation without calling prior medium scripts. The Windows player path remains `build\windows-player-stage16\ProjectAegisRTS.exe`.

Stage 28 adds integrated feature regression and player-facing QA:
...
Use `run-stage28-fast-checks.ps1` while iterating on feature-route validation, the hidden F10 QA overlay, PCDesktop sidebar regressions, Stage27.1 placement coverage, or docs/tooling. Use `run-stage28-medium-checks.ps1` before local commits; it stays flat and calls direct Stage 27.1, Stage 4/5, and Stage 28 validation dependencies. The Windows player path remains `build\windows-player-stage16\ProjectAegisRTS.exe`.

Stage 28.1 fixes the PC safe-area layout and full-gate recursion:
...
Use `run-stage28-1-fast-checks.ps1` while iterating on PCDesktop camera/sidebar layout, diagonal movement, or Stage 28.1 validators. Use `run-stage28-1-medium-checks.ps1` before local commits; it stays flat and does not call prior medium scripts. Use `run-stage28-1-checks.ps1` for final Stage 0-through-Stage 28.1 acceptance.

Stage 32 adds terrain set dressing and source-art ingestion:

```powershell
.\tools\run-stage32-terrain-art-ingestion.ps1
.\tools\run-unity-stage32-validation.ps1
.\tools\run-stage32-medium-checks.ps1
.\tools\run-stage32-player-facing-checks.ps1 -SkipPlayerBuild
.\tools\run-stage32-checks.ps1
```

Stage32.5 imports real Batch01 source terrain art:
...
Batch01 transparent PNGs under `unity\Assets\Rts\Art\Source\Terrain\Batch01\individual` are runtime source art for Stage32.5. Their `_card.png` companions are fallback/reference files. The Stage32.5 importer creates materials, textured mesh-plane prefabs, a terrain art manifest, player-facing mapped replacement prefabs, and `Assets/Rts/Scenes/Stage32_5_TerrainArtBatch01Review.unity`.

Stage32.6 corrects the terrain-art runtime path:

```powershell
.\tools\run-unity-stage32-6-validation.ps1
.\tools\run-unity-stage32-6-final-terrain-mesh-validation.ps1
.\tools\run-stage32-6-fast-checks.ps1
.\tools\run-stage32-6-medium-checks.ps1
.\tools\run-stage32-6-player-facing-checks.ps1 -SkipPlayerBuild
.\tools\run-stage32-6-checks.ps1
```

Stage32.6 applies to the rejected concept-sheet/crop-card runtime path, not to the Stage32.5 transparent source PNG package. FinalMeshBatch01 adds directly imported OBJ terrain pieces under `unity\Assets\Rts\Art\Source\Terrain\FinalMeshBatch01`, creates Unity materials/prefabs, maps the grass ground and blue resource examples into safe Stage32 player-facing definitions, and keeps the old generated proxy kit available through `.\tools\run-stage32-terrain-kit-generator.ps1` for debug/review and fallback only.
...
Use the Stage 33 generator after tank source overlay edits or before reviewing the tank prefabs in Unity. It generates and validates the light, medium, and heavy tank production-source prefabs, updates compatible `ActorVisualDefinition` assets while preserving generated blockout fallbacks, creates the Stage 33 review scene, and writes `docs\STAGE33_TANK_SOURCE_REPORT.md`. For pre-commit or broad regression confidence, continue to use the highest available medium gate, currently `.\tools\run-stage32-medium-checks.ps1`; medium/full validation remains flat and non-recursive.
...
- `Assets/Rts/Scenes/Stage32_6_TerrainArtIntegrationReview.unity`: Stage 32.6 terrain runtime-art correction review scene with mesh/material terrain prefabs and no reference-sheet cards.
- `Assets/Rts/Scenes/Stage32_6_FinalTerrainMeshReview.unity`: Stage 32.6 final terrain mesh review scene for the imported grass/dirt ground and blue resource OBJ pieces.
- `Assets/Rts/Scenes/Stage33_TankSourceReview.unity`: Stage 33 tank source review scene for light, medium, and heavy tank source/proxy prefabs with sockets, pivots, LODs, and motion hooks.
...
- Stage 25 engineer/transport design: `docs/STAGE25_ENGINEER_TRANSPORT_DESIGN.md`
- Stage 25 report: `docs/STAGE25_REPORT.md`
- Stage 26 air/naval design: `docs/STAGE26_AIR_NAVAL_FOUNDATION_DESIGN.md`
- Stage 26 report: `docs/STAGE26_REPORT.md`
- Stage 27 skirmish playability design: `docs/STAGE27_SKIRMISH_PLAYABILITY_DESIGN.md`
- Stage 27 report: `docs/STAGE27_REPORT.md`
- Stage 27.1 PC placement UX fix: `docs/STAGE27_1_PC_PLACEMENT_UX_FIX.md`
- Stage 28 integrated playtest stabilization: `docs/STAGE28_REPORT.md`
...
- Stage 32 terrain set dressing report: `docs/STAGE32_REPORT.md`
...
- Stage 32 generated visual QA report: `docs/STAGE32_VISUAL_QA_REPORT.md`
- Stage 32 terrain asset replacement overlay: `docs/STAGE32_TERRAIN_ASSET_REPLACEMENT.md`
- Stage 32 terrain art ingestion: `docs/STAGE32_TERRAIN_ART_INGESTION.md`
- Stage 32 generated terrain kit report: `docs/STAGE32_GENERATED_TERRAIN_KIT_REPORT.md`
- Stage 32 generated terrain library index: `docs/STAGE32_TERRAIN_LIBRARY_INDEX.md`
- Stage 32 generated terrain QA report: `docs/STAGE32_TERRAIN_QA_REPORT.md`
- Stage 32.6 terrain correction: `docs/STAGE32_6_TERRAIN_ART_INTEGRATION_CORRECTION.md`
- Stage 32.6 runtime asset rules: `docs/STAGE32_6_TERRAIN_RUNTIME_ASSET_RULES.md`
- Stage 32.6 report: `docs/STAGE32_6_REPORT.md`
- Stage 32.6 final terrain meshes: `docs/STAGE32_6_FINAL_TERRAIN_MESHES_README.md`
- Stage 32.6 final terrain mesh report: `docs/STAGE32_6_FINAL_TERRAIN_MESHES_REPORT.md`
- Stage 33 tank source integration: `docs/STAGE33_TANK_SOURCE_INTEGRATION.md`
- Stage 33 tank source report: `docs/STAGE33_TANK_SOURCE_REPORT.md`
- Overnight visual quality report: `docs/OVERNIGHT_VISUAL_QUALITY_REPORT.md`
