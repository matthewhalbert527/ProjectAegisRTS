...
- Scatter must support biome and topology: trees on suitable terrain, rocks near cliffs, rubble near craters, low-profile grass/pebble ground litter in open fields, and no clutter in start/build zones.
...
- Production preview now hides debug overlays by default and uses smaller/mixed terrain chunks, slower world-continuous terrain UVs, macro and micro terrain detail overlays, strip-merged water with patch-based edge detail, weathered bridge prototypes with segmented rails, abutments, deck dust, bank contact dressing, road-edge breakup overlays, capped resource glints, procedural terrain textures, low-profile ground litter, layered surface detail, and clustered scatter.
