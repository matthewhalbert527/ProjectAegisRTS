...
- Visible chunk blocks: `AegisTerrainLayerCompiler` used 16x16 dominant-role quads, so mixed terrain areas became large square patches. This pass changes production terrain to 4-cell chunks, breaks mixed chunks down to per-cell patches, suppresses isolated rough cells in production preview, and uses soft transition blend overlays instead of opaque transition strips.
...
- Terrain texture repetition: production terrain now uses smaller semantic patches and material tiling rather than stretching one large debug-like dominant role over 16 cells. Production preview also remaps cliff/rough river shoulders to lighter dirt surfaces and suppresses isolated rough cells so imported cliff pieces, wet-bank meshes, and blend overlays carry the edge detail instead of flat gray masks.
...
- Production transition masks use transparent grass/dirt/gravel/mud blend roles with deterministic width and offset variation.
- Isolated rough/cliff cells surrounded by soft terrain are visually folded back into grass/dirt in production preview.
