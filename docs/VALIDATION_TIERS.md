...
| Medium | `.\tools\run-stage32-5-medium-checks.ps1` | You are preparing a local Stage 32.5 terrain-source-art commit. | Runs `Rts.Core` tests, builds/copies the Unity DLL, runs direct Stage 4/5 preservation checks, imports and validates Batch01 terrain source art, runs player-facing source-art checks with the player build skipped, audits recursion, checks `Rts.Core` for `UnityEngine`, and runs `git diff --check`. |
