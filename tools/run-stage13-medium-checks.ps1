...
Write-Host 'Scope: Rts.Core tests, Unity DLL build, direct Stage 12 medium validation as the immediate dependency, Stage 13 validation, Stage 13 Play Mode smoke or live fallback, Rts.Core UnityEngine scan, and git diff whitespace check.'
Write-Host 'This medium tier avoids replaying the full Stage 0-through-Stage 13 acceptance chain. Use run-stage13-checks.ps1 for final acceptance.'
...
& (Join-Path $repoRoot 'tools\run-stage12-medium-checks.ps1')
if ($LASTEXITCODE -ne 0) {
    throw "run-stage12-medium-checks.ps1 failed with exit code $LASTEXITCODE."
}
