...
Write-ValidationSection 'Stage 19 medium checks'
Write-Host 'Purpose: pre-commit confidence for mission flow tuning.'
Write-Host 'Scope: Rts.Core tests, one Unity DLL build, direct Stage 18.5 Unity/player-facing validation, Stage 19 validation and smoke, Stage 19 player-facing checks, medium recursion audit, Rts.Core UnityEngine scan, and git diff whitespace check.'
Write-Host 'This medium tier does not call prior medium checks. Use run-stage19-checks.ps1 for full Stage 0-through-Stage 19 acceptance.'
...
Write-ValidationSection 'Direct Stage 18.5 Unity validation'
...
Write-ValidationSection 'Direct Stage 18.5 player-facing validation'
$stage18_5PlayerFacingScript = Join-Path $repoRoot 'tools\run-stage18-5-player-facing-checks.ps1'
& $stage18_5PlayerFacingScript -SkipPlayerBuild -SkipPlayerLog -SkipCoreBuild -SkipStage18Dependency -SkipStage18Validation -SkipStage18_5Validation
if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Stage 19 medium checks passed.'
