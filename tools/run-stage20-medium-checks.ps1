...
Write-ValidationSection 'Stage 20 medium checks'
Write-Host 'Purpose: pre-commit confidence for Stage 20 production proxy visuals and platform UI split.'
Write-Host 'Scope: Rts.Core tests/build, direct Stage19.5 Unity/player-facing validation, direct Stage4/Stage5 hand-control validation, Stage20 validation and smoke, Stage20 player-facing checks, medium recursion audit, UnityEngine-free scan, and git diff whitespace check.'
Write-Host 'This medium tier does not call prior medium checks. Use run-stage20-checks.ps1 for the slow full final acceptance gate.'
...
Write-ValidationSection 'Direct Stage 19.5 Unity validation'
...
Write-ValidationSection 'Direct Stage 19.5 player-facing validation'
& (Join-Path $repoRoot 'tools\run-stage19-5-player-facing-checks.ps1') -SkipPlayerBuild -SkipPlayerLog -SkipCoreBuild -SkipStage19Validation -SkipStage19_5Validation
if ($LASTEXITCODE -ne 0) {
...
& (Join-Path $repoRoot 'tools\run-stage20-player-facing-checks.ps1') -SkipPlayerBuild -SkipPlayerLog -SkipCoreBuild -SkipStage19_5Validation -SkipStage20Validation
if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Stage 20 medium checks passed.'
