...
Write-ValidationSection 'Stage 22 player-facing checks'
Write-Host 'Scope: core tests/build, medium recursion audit, direct Stage21.5 player-facing/build-flow validation, Stage22 Unity validation, optional Windows player build/launch/log inspection, UnityEngine-free scan, and git diff whitespace check.'
...
if ($SkipStage21_5Validation) {
    Write-Host 'Skipping Stage 21.5 player-facing dependency; caller already ran it.'
} else {
    Write-ValidationSection 'Direct Stage 21.5 player-facing validation'
    & (Join-Path $repoRoot 'tools\run-stage21-5-player-facing-checks.ps1') -SkipPlayerBuild -SkipPlayerLog -SkipCoreBuild -SkipStage21Validation
    if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Stage 22 player-facing checks passed.'
