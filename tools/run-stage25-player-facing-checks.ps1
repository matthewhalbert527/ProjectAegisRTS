...
param(
    ...
    [switch]$SkipStage24Validation,
    [switch]$SkipStage25Validation
)
...
Write-ValidationSection 'Stage 25 player-facing checks'
Write-Host 'Scope: core tests/build, medium recursion audit, direct Stage24 validation, Stage25 Unity validation, optional Windows player build/launch/log inspection, UnityEngine-free scan, and git diff whitespace check.'
...
if ($SkipStage24Validation) {
    Write-Host 'Skipping Stage 24 Unity validation; caller already ran it.'
} else {
...
if ($SkipStage25Validation) {
    Write-Host 'Skipping Stage 25 Unity validation; caller already ran it.'
} else {
...
Write-Host 'Stage 25 player-facing checks passed.'
