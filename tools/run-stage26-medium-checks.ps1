...
Write-ValidationSection 'Stage 26 medium checks'
Write-Host 'Purpose: pre-commit confidence for air/naval foundations without recursively running prior medium tiers.'
Write-Host 'Scope: Rts.Core tests/build, direct Stage25 Unity/player-facing validation, direct Stage4/Stage5 hand-control validation, Stage26 Unity validation and smoke, Stage26 player-facing checks, medium recursion audit, UnityEngine-free scan, and git diff whitespace check.'
Write-Host 'This medium tier does not call prior medium checks. Use run-stage26-checks.ps1 for the slow full final acceptance gate.'
...
Write-ValidationSection 'Direct Stage 25 Unity validation'
...
Write-ValidationSection 'Direct Stage 25 player-facing validation'
...
Write-ValidationSection 'Stage 26 Unity validation'
...
Write-ValidationSection 'Stage 26 player-facing validation'
& (Join-Path $repoRoot 'tools\run-stage26-player-facing-checks.ps1') -SkipPlayerBuild -SkipPlayerLog -SkipCoreBuild -SkipStage25Validation -SkipStage26Validation
if ($LASTEXITCODE -ne 0) {
    throw "run-stage26-player-facing-checks.ps1 failed with exit code $LASTEXITCODE."
}
...
Write-Host 'Stage 26 medium checks passed.'
