...
Write-ValidationSection 'Stage 27.1 medium checks'
Write-Host 'Purpose: pre-commit confidence for the PC placement UX regression without recursively running prior medium tiers.'
Write-Host 'Scope: Rts.Core tests/build, direct Stage27 player-facing validation, direct Stage4/Stage5 hand-control validation, Stage27.1 Unity validation and smoke, Stage27.1 player-facing checks with Windows build skipped, medium recursion audit, UnityEngine-free scan, and git diff whitespace check.'
Write-Host 'This medium tier does not call prior medium checks. Use run-stage27-1-checks.ps1 for the slow full final acceptance gate.'
...
Write-ValidationSection 'Direct Stage 27 player-facing validation'
& (Join-Path $repoRoot 'tools\run-stage27-player-facing-checks.ps1') -SkipPlayerBuild -SkipPlayerLog -SkipCoreBuild -SkipStage26Validation
if ($LASTEXITCODE -ne 0) {
...
Write-ValidationSection 'Stage 27.1 Unity validation'
...
Write-Host 'Stage 27.1 medium checks passed.'
