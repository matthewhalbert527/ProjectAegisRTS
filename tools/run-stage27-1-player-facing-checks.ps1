...
Write-ValidationSection 'Stage 27.1 player-facing checks'
Write-Host 'Scope: core tests/build, medium recursion audit, direct Stage27 player-facing validation, Stage4/Stage5 hand controls, Stage27.1 Unity placement validation, optional Windows player build/launch/log inspection, UnityEngine-free scan, and git diff whitespace check.'
...
if ($SkipStage27Validation) {
    Write-Host 'Skipping Stage 27 player-facing validation; caller already ran it.'
} else {
...
if ($SkipStage27_1Validation) {
    Write-Host 'Skipping Stage 27.1 Unity validation; caller already ran it.'
} else {
...
if ($SkipPlayerBuild) {
    ...
    Invoke-PlayerLaunchSmoke -ExePath $exePath -Label 'Stage 27.1 Windows player'
}
...
Write-Host 'Stage 27.1 player-facing checks passed.'
