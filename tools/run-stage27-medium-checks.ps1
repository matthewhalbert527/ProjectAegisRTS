...
Write-ValidationSection 'Stage 27 medium checks'
Write-Host 'Purpose: pre-commit confidence for skirmish playability without recursively running prior medium tiers.'
Write-Host 'Scope: Rts.Core tests/build, direct Stage26 Unity/player-facing validation, direct Stage4/Stage5 hand-control validation, Stage27 Unity validation and smoke, Stage27 player-facing checks, medium recursion audit, UnityEngine-free scan, and git diff whitespace check.'
Write-Host 'This medium tier does not call prior medium checks. Use run-stage27-checks.ps1 for the slow full final acceptance gate.'
...
Write-ValidationSection 'Direct Stage 26 player-facing validation'
...
& (Join-Path $repoRoot 'tools\run-stage27-player-facing-checks.ps1') -SkipPlayerBuild -SkipPlayerLog -SkipCoreBuild -SkipStage26Validation -SkipStage27Validation
if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Stage 27 medium checks passed.'
