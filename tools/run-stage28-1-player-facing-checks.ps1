...
Write-ValidationSection 'Stage 28.1 player-facing checks'
Write-Host 'Scope: core tests/build, recursion audits, direct Stage28 player-facing validation, Stage28.1 layout validation, optional Windows player build/launch/log inspection, UnityEngine-free scan, and git diff whitespace check.'
...
if ($SkipStage28Validation) {
    Write-Host 'Skipping Stage 28 player-facing validation; caller already ran it.'
} else {
    Write-ValidationSection 'Direct Stage 28 player-facing validation'
    & (Join-Path $repoRoot 'tools\run-stage28-player-facing-checks.ps1') -SkipCoreBuild -SkipPlayerBuild -SkipPlayerLog
    if ($LASTEXITCODE -ne 0) {
...
if ($SkipStage28_1Validation) {
    Write-Host 'Skipping Stage 28.1 Unity validation; caller already ran it.'
} else {
    Write-ValidationSection 'Stage 28.1 layout/player-facing Unity validation'
...
if ($SkipPlayerBuild) {
    ...
    Invoke-PlayerLaunchSmoke -ExePath $exePath -Arguments @('-screen-width', '1600', '-screen-height', '900', '-screen-fullscreen', '0') -Label 'Stage 28.1 Windows player'
}
...
Write-Host 'Stage 28.1 player-facing checks passed.'
