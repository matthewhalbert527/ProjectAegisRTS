...
Write-ValidationSection 'Stage 29 medium checks'
Write-Host 'Purpose: pre-commit confidence for the Stage 29 visual-quality pass without recursive medium chaining.'
Write-Host 'Scope: Rts.Core tests/build, direct Stage28/28.1 preservation, Stage4/Stage5 hand controls, Stage29 visual validation, Stage29 player-facing checks with Windows build skipped, recursion audits, UnityEngine-free scan, and git diff whitespace check.'
...
Write-ValidationSection 'Direct Stage 28 Unity validation'
...
Write-ValidationSection 'Stage 29 Unity validation'
...
& (Join-Path $repoRoot 'tools\run-stage29-player-facing-checks.ps1') -SkipCoreBuild -SkipPlayerBuild -SkipPlayerLog -SkipStage28_1Validation -SkipStage29Validation
if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Stage 29 medium checks passed.'
