...
Write-ValidationSection 'Stage 30 player-facing checks'
Write-Host 'Scope: core tests/build, recursion audits, Stage29 preservation, Stage30 readability validation, optional Windows player build/launch/log inspection, UnityEngine-free scan, and git diff whitespace check.'
...
if ($SkipStage29Validation) {
    Write-Host 'Skipping Stage 29 player-facing validation; caller already ran it.'
} else {
    Write-ValidationSection 'Direct Stage 29 player-facing validation'
    & (Join-Path $repoRoot 'tools\run-stage29-player-facing-checks.ps1') -SkipCoreBuild -SkipPlayerBuild -SkipPlayerLog
    if ($LASTEXITCODE -ne 0) {
        throw "run-stage29-player-facing-checks.ps1 failed with exit code $LASTEXITCODE."
    }
}

if ($SkipStage30Validation) {
    Write-Host 'Skipping Stage 30 Unity readability validation; caller already ran it.'
} else {
...
if ($SkipPlayerBuild) {
    ...
    Invoke-PlayerLaunchSmoke -ExePath $exePath -Arguments @('-screen-width', '1600', '-screen-height', '900', '-screen-fullscreen', '0') -Label 'Stage 30 Windows player'
}
...
Write-Host 'Stage 30 player-facing checks passed.'
