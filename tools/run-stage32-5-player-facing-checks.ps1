...
param(
    ...
    [switch]$SkipStage32_5Validation,
...
Write-ValidationSection 'Stage 32.5 player-facing checks'
Write-Host 'Scope: core tests/build, Batch01 source-art import, PC sidebar preservation, QuestXR hand-control preservation, optional Windows player build/log, UnityEngine-free scan, and whitespace.'
...
if (-not $SkipSafetyDependencies) {
    ...
    Write-Host 'Skipping safety dependency rechecks; caller already ran them.'
}

if ($SkipStage32_5Validation) {
    Write-Host 'Skipping Stage 32.5 Unity validation; caller already ran it.'
} else {
...
if ($SkipPlayerBuild) {
    ...
    Invoke-PlayerLaunchSmoke -ExePath $exePath -Arguments @('-screen-width', '1600', '-screen-height', '900', '-screen-fullscreen', '0') -Label 'Stage 32.5 Windows player'
}
...
Write-Host 'Stage 32.5 player-facing checks passed.'
