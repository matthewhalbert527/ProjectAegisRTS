...
Write-ValidationSection 'Stage 32.6 full checks'
Write-Host 'Scope: final acceptance for terrain art integration correction, including core tests/build, audits, Stage4/Stage5, Stage32.6 validation, Windows player build/log, UnityEngine-free scan, and whitespace.'
...
& (Join-Path $repoRoot 'tools\run-stage32-6-player-facing-checks.ps1') -SkipCoreBuild -SkipStage32_6Validation
if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Stage 32.6 full checks passed.'
