...
if ($SkipStage32Validation) {
    Write-Host 'Skipping Stage 32 Unity validation; caller already ran it.'
} else {
...
if ($SkipPlayerBuild) {
    ...
    Invoke-PlayerLaunchSmoke -ExePath $exePath -Arguments @('-screen-width', '1600', '-screen-height', '900', '-screen-fullscreen', '0') -Label 'Stage 32 Windows player'
}
...
Write-Host 'Stage 32 player-facing checks passed.'
