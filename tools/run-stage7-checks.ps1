...
function Normalize-UnityGeneratedFiles {
    ...
    $profileRoots = @(
        'unity\Assets\Rts\ScriptableObjects\MotionProfiles',
        'unity\Assets\Rts\ScriptableObjects\BuildingProfiles'
    )

    foreach ($relativeRoot in $profileRoots) {
        $profilePath = Join-Path $RepoRoot $relativeRoot
...
Write-Host 'Running Stage 6 checks.'
& (Join-Path $repoRoot 'tools\run-stage6-checks.ps1')
if ($LASTEXITCODE -ne 0) {
    throw "run-stage6-checks.ps1 failed with exit code $LASTEXITCODE."
}

Write-Host 'Running Stage 7 Unity validation.'
...
Write-Host 'Stage 7 checks passed.'
