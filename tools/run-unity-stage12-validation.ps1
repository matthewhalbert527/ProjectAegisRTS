...
function Test-Stage12SceneFileLive {
    param([string]$ScenePath)

    if (-not (Test-Path -LiteralPath $ScenePath)) {
        throw "Stage 12 scene does not exist and Unity is already open, so it cannot be generated in batchmode: $ScenePath"
    }
    ...
    $required = @(
        ...
        'm_Name: EventSystem',
        'm_Name: Canvas',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Ai.AiIntentRenderSystem',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Ai.AiPlanTimelineView',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.UI.Common.AiDebugHud'
    )

    foreach ($pattern in $required) {
        if (-not ($lines | Select-String -Pattern ([regex]::Escape($pattern)) -Quiet)) {
            throw "Stage 12 scene live validation failed; missing '$pattern'."
        }
    }
}

function Normalize-Stage12GeneratedFiles {
    param([string]$RepoRoot)

    $paths = @(
        'unity\Assets\Rts\Scenes\Stage12_AISkirmishFoundation.unity',
        'unity\Assets\Rts\Scenes\Stage12_AISkirmishFoundation.unity.meta',
        'unity\Assets\Rts\Scripts\Rendering\Ai.meta',
        'unity\Assets\Rts\Scripts\UI\Common\AiDebugHud.cs.meta',
        'unity\Assets\Rts\Editor\Stage12SceneCreator.cs.meta',
        'unity\Assets\Rts\Editor\Stage12SceneValidator.cs.meta',
        'unity\Assets\Rts\Editor\Stage12PlayModeSmokeValidator.cs.meta',
    ...
    $roots = @(
        'unity\Assets\Rts\Scripts\Rendering\Ai'
    )
...
$scenePath = Join-Path $unityProject 'Assets\Rts\Scenes\Stage12_AISkirmishFoundation.unity'
...
Write-Host "Stage 12 scene: $scenePath"
...
if ($openUnity.Count -gt 0) {
    Write-Warning 'Unity Editor is already open for this project. Using Stage 12 live scene/log validation fallback; full batch Play Mode automation is skipped because Unity owns the project lock.'
    Test-Stage12SceneFileLive -ScenePath $scenePath
    Normalize-Stage12GeneratedFiles -RepoRoot $repoRoot
    ...
    Write-Host 'Stage 12 live validation passed. Play Mode smoke used fallback.'
    Write-Host "Scene path: $scenePath"
    return
}

Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage12SceneCreator.CreateStage12SceneBatch' -LogPath (Join-Path $logRoot 'stage12-create.log') -SuccessPattern 'Created Stage 12 scene at Assets/Rts/Scenes/Stage12_AISkirmishFoundation.unity' -TimeoutMinutes 15
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage12SceneValidator.ValidateStage12SceneBatch' -LogPath (Join-Path $logRoot 'stage12-validate.log') -SuccessPattern 'Stage 12 scene validation passed.' -TimeoutMinutes 12
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage12PlayModeSmokeValidator.RunStage12PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage12-playmode-smoke.log') -SuccessPattern 'Stage 12 play mode smoke validation passed.' -TimeoutMinutes 12

Normalize-Stage12GeneratedFiles -RepoRoot $repoRoot

Write-Host "Stage 12 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 12 Play Mode smoke fully ran in batchmode.'
Write-Host "Scene path: $scenePath"
