...
function Test-Stage13SceneFileLive {
    param([string]$ScenePath)

    if (-not (Test-Path -LiteralPath $ScenePath)) {
        throw "Stage 13 scene does not exist and Unity is already open, so it cannot be generated in batchmode: $ScenePath"
    }
    ...
    $required = @(
        ...
        'm_Name: EventSystem',
        'm_Name: Canvas',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Map.TerrainDebugRenderer',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Map.PathDebugRenderer',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Map.MapAuthoringOverlay',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.UI.Common.MapValidationDebugHud'
    )

    foreach ($pattern in $required) {
        if (-not ($lines | Select-String -Pattern ([regex]::Escape($pattern)) -Quiet)) {
            throw "Stage 13 scene live validation failed; missing '$pattern'."
        }
    }
}

function Normalize-Stage13GeneratedFiles {
    param([string]$RepoRoot)

    $paths = @(
        'unity\Assets\Rts\Scenes\Stage13_MapTerrainPathing.unity',
        'unity\Assets\Rts\Scenes\Stage13_MapTerrainPathing.unity.meta',
        'unity\Assets\Rts\Scripts\Rendering\Map.meta',
        'unity\Assets\Rts\Scripts\UI\Common\MapValidationDebugHud.cs.meta',
        'unity\Assets\Rts\Editor\Stage13SceneCreator.cs.meta',
        'unity\Assets\Rts\Editor\Stage13SceneValidator.cs.meta',
        'unity\Assets\Rts\Editor\Stage13PlayModeSmokeValidator.cs.meta',
    ...
    $roots = @(
        'unity\Assets\Rts\Scripts\Rendering\Map'
    )
...
$scenePath = Join-Path $unityProject 'Assets\Rts\Scenes\Stage13_MapTerrainPathing.unity'
...
if ($openUnity.Count -gt 0) {
    Write-Warning 'Unity Editor is already open for this project. Using Stage 13 live scene/log validation fallback; full batch Play Mode automation is skipped because Unity owns the project lock.'
    Test-Stage13SceneFileLive -ScenePath $scenePath
    Normalize-Stage13GeneratedFiles -RepoRoot $repoRoot
    ...
    Write-Host 'Stage 13 live validation passed. Play Mode smoke used fallback.'
    Write-Host "Scene path: $scenePath"
    return
}

Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage13SceneCreator.CreateStage13SceneBatch' -LogPath (Join-Path $logRoot 'stage13-create.log') -SuccessPattern 'Created Stage 13 scene at Assets/Rts/Scenes/Stage13_MapTerrainPathing.unity' -TimeoutMinutes 15
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage13SceneValidator.ValidateStage13SceneBatch' -LogPath (Join-Path $logRoot 'stage13-validate.log') -SuccessPattern 'Stage 13 scene validation passed.' -TimeoutMinutes 12
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage13PlayModeSmokeValidator.RunStage13PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage13-playmode-smoke.log') -SuccessPattern 'Stage 13 play mode smoke validation passed.' -TimeoutMinutes 12

Normalize-Stage13GeneratedFiles -RepoRoot $repoRoot

Write-Host "Stage 13 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 13 Play Mode smoke fully ran in batchmode.'
Write-Host "Scene path: $scenePath"
