...
function Test-Stage14SceneFileLive {
    param([string]$ScenePath)

    if (-not (Test-Path -LiteralPath $ScenePath)) {
        throw "Stage 14 scene does not exist and Unity is already open, so it cannot be generated in batchmode: $ScenePath"
    }
    ...
    $required = @(
        ...
        'm_Name: EventSystem',
        'm_Name: Canvas',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Feedback.FeedbackEventBus',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Feedback.FeedbackProfileLibrary',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Feedback.AudioFeedbackController',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Feedback.VfxFeedbackController',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Feedback.UiFeedbackController',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Feedback.HapticFeedbackAdapter',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Feedback.FeedbackDebugHud'
    )

    foreach ($pattern in $required) {
        if (-not ($lines | Select-String -Pattern ([regex]::Escape($pattern)) -Quiet)) {
            throw "Stage 14 scene live validation failed; missing '$pattern'."
        }
    }
}

function Normalize-Stage14GeneratedFiles {
    param([string]$RepoRoot)

    $paths = @(
        'unity\Assets\Rts\Scenes\Stage14_FeedbackPolish.unity',
        'unity\Assets\Rts\Scenes\Stage14_FeedbackPolish.unity.meta',
        'unity\Assets\Rts\Scripts\Feedback.meta',
        'unity\Assets\Rts\Editor\Stage14FeedbackProfileAssetCreator.cs.meta',
        'unity\Assets\Rts\Editor\Stage14SceneCreator.cs.meta',
        'unity\Assets\Rts\Editor\Stage14SceneValidator.cs.meta',
        'unity\Assets\Rts\Editor\Stage14PlayModeSmokeValidator.cs.meta',
    ...
    $roots = @(
        'unity\Assets\Rts\Scripts\Feedback',
...
$scenePath = Join-Path $unityProject 'Assets\Rts\Scenes\Stage14_FeedbackPolish.unity'
...
Write-Host "Stage 14 scene: $scenePath"
...
if ($openUnity.Count -gt 0) {
    Write-Warning 'Unity Editor is already open for this project. Using Stage 14 live scene/log validation fallback; full batch Play Mode automation is skipped because Unity owns the project lock.'
    Test-Stage14SceneFileLive -ScenePath $scenePath
    Normalize-Stage14GeneratedFiles -RepoRoot $repoRoot
    ...
    Write-Host 'Stage 14 live validation passed. Play Mode smoke used fallback.'
    Write-Host "Scene path: $scenePath"
    return
}

Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage14FeedbackProfileAssetCreator.CreateFeedbackProfilesBatch' -LogPath (Join-Path $logRoot 'stage14-feedback-profiles.log') -SuccessPattern 'Stage 14 feedback profiles updated' -TimeoutMinutes 12
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage14SceneCreator.CreateStage14SceneBatch' -LogPath (Join-Path $logRoot 'stage14-create.log') -SuccessPattern 'Created Stage 14 scene at Assets/Rts/Scenes/Stage14_FeedbackPolish.unity' -TimeoutMinutes 15
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage14SceneValidator.ValidateStage14SceneBatch' -LogPath (Join-Path $logRoot 'stage14-validate.log') -SuccessPattern 'Stage 14 scene validation passed.' -TimeoutMinutes 12
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage14PlayModeSmokeValidator.RunStage14PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage14-playmode-smoke.log') -SuccessPattern 'Stage 14 play mode smoke validation passed.' -TimeoutMinutes 15

Normalize-Stage14GeneratedFiles -RepoRoot $repoRoot

Write-Host "Stage 14 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 14 Play Mode smoke fully ran in batchmode.'
Write-Host "Scene path: $scenePath"
