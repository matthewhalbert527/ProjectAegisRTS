...
Write-Host 'Stage 17 scenes: Stage16_5_Boot and Stage16_PlayableVerticalSlice'
...
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage17SceneCreator.ConfigureStage17Batch' -LogPath (Join-Path $logRoot 'stage17-configure.log') -SuccessPattern 'Stage 17 player-facing polish configured.' -TimeoutMinutes 18
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage17SceneValidator.ValidateStage17SceneBatch' -LogPath (Join-Path $logRoot 'stage17-validate.log') -SuccessPattern 'Stage 17 scene validation passed.' -TimeoutMinutes 12
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage17PlayModeSmokeValidator.RunStage17PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage17-playmode-smoke.log') -SuccessPattern 'Stage 17 play mode smoke validation passed.' -TimeoutMinutes 20

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 17 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 17 Play Mode smoke fully ran in batchmode.'
