...
function Invoke-UnityBatchStage18_5 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 18.5 scene: Stage16_PlayableVerticalSlice with doubled fine placement grid.'
...
Invoke-UnityBatchStage18_5 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage18SceneCreator.ConfigureStage18Batch' -LogPath (Join-Path $logRoot 'stage18-5-configure.log') -SuccessPattern 'Stage 18 tester playability pass configured.' -TimeoutMinutes 18
Invoke-UnityBatchStage18_5 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage18_5FineGridValidator.ValidateStage18_5FineGridBatch' -LogPath (Join-Path $logRoot 'stage18-5-validate.log') -SuccessPattern 'Stage 18.5 fine placement grid validation passed.' -TimeoutMinutes 14
Invoke-UnityBatchStage18_5 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage18_5PlayModeSmokeValidator.RunStage18_5PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage18-5-playmode-smoke.log') -SuccessPattern 'Stage 18.5 play mode smoke validation passed.' -TimeoutMinutes 24

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 18.5 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 18.5 Play Mode smoke fully ran in batchmode.'
