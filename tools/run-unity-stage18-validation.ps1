...
Write-Host 'Stage 18 scenes: Stage16_5_Boot and Stage16_PlayableVerticalSlice'
...
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage18SceneCreator.ConfigureStage18Batch' -LogPath (Join-Path $logRoot 'stage18-configure.log') -SuccessPattern 'Stage 18 tester playability pass configured.' -TimeoutMinutes 18
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage18SceneValidator.ValidateStage18SceneBatch' -LogPath (Join-Path $logRoot 'stage18-validate.log') -SuccessPattern 'Stage 18 scene validation passed.' -TimeoutMinutes 14
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage18PlayModeSmokeValidator.RunStage18PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage18-playmode-smoke.log') -SuccessPattern 'Stage 18 play mode smoke validation passed.' -TimeoutMinutes 24

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 18 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 18 Play Mode smoke fully ran in batchmode.'
