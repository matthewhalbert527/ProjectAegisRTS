...
function Invoke-UnityBatchStage22 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 22 command matrix Unity validation.'
...
Invoke-UnityBatchStage22 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage22CommandMatrixValidator.ValidateStage22CommandMatrixBatch' -LogPath (Join-Path $logRoot 'stage22-command-matrix.log') -SuccessPattern 'Stage 22 command matrix validation passed.' -TimeoutMinutes 28
Invoke-UnityBatchStage22 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage22PlayModeSmokeValidator.RunStage22PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage22-playmode-smoke.log') -SuccessPattern 'Stage 22 play mode smoke validation passed.' -TimeoutMinutes 32

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 22 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 22 Play Mode smoke verified command bar buttons, PC sidebar layout, hidden debug/placement defaults, and deterministic command bridge actions for attack-move, guard, patrol, scatter, deploy, and stop.'
