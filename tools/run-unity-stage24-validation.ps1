...
function Invoke-UnityBatchStage24 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 24 tech tree/support power Unity validation.'
...
Invoke-UnityBatchStage24 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage24TechSupportValidator.ValidateStage24TechSupportBatch' -LogPath (Join-Path $logRoot 'stage24-tech-support.log') -SuccessPattern 'Stage 24 tech/support validation passed.' -TimeoutMinutes 34
Invoke-UnityBatchStage24 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage24PlayModeSmokeValidator.RunStage24PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage24-playmode-smoke.log') -SuccessPattern 'Stage 24 play mode smoke validation passed.' -TimeoutMinutes 40

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 24 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 24 Play Mode smoke verified support-power panel wiring, Reveal Scan activation, cooldown gating, and hidden debug defaults.'
