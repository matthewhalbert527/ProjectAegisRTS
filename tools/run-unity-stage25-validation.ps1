...
function Invoke-UnityBatchStage25 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 25 engineer/capture/transport Unity validation.'
...
Invoke-UnityBatchStage25 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage25EngineerTransportValidator.ValidateStage25EngineerTransportBatch' -LogPath (Join-Path $logRoot 'stage25-engineer-transport.log') -SuccessPattern 'Stage 25 engineer/transport validation passed.' -TimeoutMinutes 38
Invoke-UnityBatchStage25 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage25PlayModeSmokeValidator.RunStage25PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage25-playmode-smoke.log') -SuccessPattern 'Stage 25 play mode smoke validation passed.' -TimeoutMinutes 44

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 25 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 25 Play Mode smoke verified capture routing, load/unload routing, transport snapshots, and hidden debug defaults.'
