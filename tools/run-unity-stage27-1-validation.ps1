...
function Invoke-UnityBatchStage27_1 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 27.1 PC building placement UX Unity validation.'
...
Invoke-UnityBatchStage27_1 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage27_1PlacementUxValidator.ValidateStage27_1PlacementUxBatch' -LogPath (Join-Path $logRoot 'stage27-1-placement-ux.log') -SuccessPattern 'Stage 27.1 placement UX validation passed.' -TimeoutMinutes 45
Invoke-UnityBatchStage27_1 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage27_1PlayModeSmokeValidator.RunStage27_1PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage27-1-playmode-smoke.log') -SuccessPattern 'Stage 27.1 play mode smoke validation passed.' -TimeoutMinutes 55

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 27.1 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 27.1 smoke verified PCDesktop sidebar placement, hidden BoardPlacementHud, fine-grid preview, Esc cancel, pause resume, and Quest board setup availability.'
