...
function Invoke-UnityBatchStage27 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 27 skirmish playability Unity validation.'
...
Invoke-UnityBatchStage27 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage27SkirmishPlayabilityValidator.ValidateStage27SkirmishPlayabilityBatch' -LogPath (Join-Path $logRoot 'stage27-skirmish-playability.log') -SuccessPattern 'Stage 27 skirmish playability validation passed.' -TimeoutMinutes 48
Invoke-UnityBatchStage27 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage27PlayModeSmokeValidator.RunStage27PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage27-playmode-smoke.log') -SuccessPattern 'Stage 27 play mode smoke validation passed.' -TimeoutMinutes 55

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 27 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 27 Play Mode smoke verified difficulty preferences, AI attack pressure, restart, right sidebar preservation, and hidden debug defaults.'
