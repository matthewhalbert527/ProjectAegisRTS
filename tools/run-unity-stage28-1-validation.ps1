...
function Invoke-UnityBatchStage28_1 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 28.1 PC safe-area layout and flattened full-gate Unity validation.'
...
Invoke-UnityBatchStage28_1 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage28_1LayoutValidator.ValidateStage28_1LayoutBatch' -LogPath (Join-Path $logRoot 'stage28-1-layout.log') -SuccessPattern 'Stage 28.1 layout validation passed.' -TimeoutMinutes 60
Invoke-UnityBatchStage28_1 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage28_1PlayModeSmokeValidator.RunStage28_1PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage28-1-playmode-smoke.log') -SuccessPattern 'Stage 28.1 play mode smoke validation passed.' -TimeoutMinutes 65

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 28.1 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 28.1 verified PC safe gameplay viewport, board/sidebar non-overlap, minimap/sidebar layout, placement HUD separation, and clean smoke execution.'
