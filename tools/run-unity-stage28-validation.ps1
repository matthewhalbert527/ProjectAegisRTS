...
function Invoke-UnityBatchStage28 {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 28 integrated feature regression Unity validation.'
...
Invoke-UnityBatchStage28 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage28FeatureRegressionValidator.ValidateStage28FeatureRegressionBatch' -LogPath (Join-Path $logRoot 'stage28-feature-regression.log') -SuccessPattern 'Stage 28 feature regression validation passed.' -TimeoutMinutes 60
Invoke-UnityBatchStage28 -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage28PlayModeSmokeValidator.RunStage28PlayModeSmokeBatch' -LogPath (Join-Path $logRoot 'stage28-playmode-smoke.log') -SuccessPattern 'Stage 28 play mode smoke validation passed.' -TimeoutMinutes 65

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot

Write-Host "Stage 28 Unity validation passed. Logs: $logRoot"
Write-Host 'Stage 28 verified PCDesktop sidebar, QuestXR control preservation, feature route coverage, Stage27.1 placement HUD separation, hidden QA overlay, and integrated smoke commands.'
