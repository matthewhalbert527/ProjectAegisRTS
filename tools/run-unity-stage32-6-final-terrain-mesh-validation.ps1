...
function Invoke-UnityBatchStage32_6FinalMesh {
    param(
        [string]$UnityEditor,
        [string]$UnityProject,
...
Write-Host 'Stage 32.6 final terrain mesh Batch01 Unity validation.'
...
Invoke-UnityBatchStage32_6FinalMesh -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage32_6FinalTerrainMeshBatch01Importer.ImportFinalMeshBatch01Batch' -LogPath (Join-Path $logRoot 'stage32-6-final-terrain-mesh-import.log') -SuccessPattern 'Stage 32.6 final terrain mesh Batch01 import completed.' -TimeoutMinutes 90
Invoke-UnityBatchStage32_6FinalMesh -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage32_6FinalTerrainMeshBatch01Importer.CreateFinalMeshReviewSceneBatch' -LogPath (Join-Path $logRoot 'stage32-6-final-terrain-mesh-review-scene.log') -SuccessPattern 'Stage 32.6 final terrain mesh Batch01 review scene created.' -TimeoutMinutes 90
Invoke-UnityBatchStage32_6FinalMesh -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage32_6FinalTerrainMeshBatch01Importer.ValidateFinalMeshBatch01Batch' -LogPath (Join-Path $logRoot 'stage32-6-final-terrain-mesh-validation.log') -SuccessPattern 'Stage 32.6 final terrain mesh Batch01 validation passed.' -TimeoutMinutes 90

Repair-UnityGeneratedValidationWhitespace -RepoRoot $repoRoot
Normalize-WhitespaceInTree -Path (Join-Path $repoRoot 'unity\Assets\Rts\Art\Source\Terrain\FinalMeshBatch01') -Include @('*.obj', '*.mtl', '*.json', '*.meta')
...
Remove-TrailingWhitespace -Path (Join-Path $repoRoot 'unity\Assets\Rts\Scenes\Stage32_6_FinalTerrainMeshReview.unity')
Remove-TrailingWhitespace -Path (Join-Path $repoRoot 'unity\Assets\Rts\Scenes\Stage32_6_FinalTerrainMeshReview.unity.meta')

Write-Host "Stage 32.6 final terrain mesh Batch01 Unity validation passed. Logs: $logRoot"
