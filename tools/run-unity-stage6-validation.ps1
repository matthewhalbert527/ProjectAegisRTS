...
function Test-Stage6SceneFileLive {
    param([string]$ScenePath)

    if (-not (Test-Path -LiteralPath $ScenePath)) {
        throw "Stage 6 scene does not exist and Unity is already open, so it cannot be generated in batchmode: $ScenePath"
    }
    ...
    $required = @(
        ...
        'm_Name: EventSystem',
        'm_Name: Canvas',
        'm_Name: Stage6 Motion Showcase',
        'm_Name: Stage6 Vehicle Motion Placeholder',
        'm_Name: Stage6 Infantry Motion Placeholder',
        'm_Name: Stage6 Aircraft Motion Placeholder',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Motion.VisualMotionProfileLibrary',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Motion.MovementPathPreview',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.Rendering.Motion.Stage6MotionShowcase',
        'Assembly-CSharp::ProjectAegisRTS.UnityClient.UI.Common.MovementDebugHud'
    )

    foreach ($pattern in $required) {
        if (-not ($lines | Select-String -Pattern ([regex]::Escape($pattern)) -Quiet)) {
            throw "Stage 6 scene live validation failed; missing '$pattern'."
        }
    }
}
...
$scenePath = Join-Path $unityProject 'Assets\Rts\Scenes\Stage6_MovementVisualization.unity'
$logRoot = Join-Path $repoRoot 'build\unity-logs'
$createLog = Join-Path $logRoot 'stage6-create.log'
$validateLog = Join-Path $logRoot 'stage6-validate.log'
$smokeLog = Join-Path $logRoot 'stage6-playmode-smoke.log'
...
Write-Host "Stage 6 scene: $scenePath"
...
if ($openUnity.Count -gt 0) {
    Write-Warning 'Unity Editor is already open for this project. Using Stage 6 live scene/log validation fallback; full batch Play Mode automation is skipped because Unity owns the project lock.'
    Test-Stage6SceneFileLive -ScenePath $scenePath
    ...
    Write-Host 'Stage 6 live validation passed.'
    return
}

Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage6SceneCreator.CreateStage6SceneBatch' -LogPath $createLog -SuccessPattern 'Created Stage 6 scene at Assets/Rts/Scenes/Stage6_MovementVisualization.unity' -TimeoutMinutes 10
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage6SceneValidator.ValidateStage6SceneBatch' -LogPath $validateLog -SuccessPattern 'Stage 6 scene validation passed.' -TimeoutMinutes 10
Invoke-UnityBatch -UnityEditor $unityEditor -UnityProject $unityProject -ExecuteMethod 'ProjectAegisRTS.UnityClient.EditorTools.Stage6PlayModeSmokeValidator.RunStage6PlayModeSmokeBatch' -LogPath $smokeLog -SuccessPattern 'Stage 6 play mode smoke validation passed.' -TimeoutMinutes 10

Remove-TrailingWhitespace -Path $scenePath
if (Test-Path -LiteralPath "$scenePath.meta") {
    Remove-TrailingWhitespace -Path "$scenePath.meta"
}
Get-ChildItem -LiteralPath (Join-Path $unityProject 'Assets\Rts\ScriptableObjects\MotionProfiles') -Filter '*.asset' -ErrorAction SilentlyContinue |
    ForEach-Object { Remove-TrailingWhitespace -Path $_.FullName }
Get-ChildItem -LiteralPath (Join-Path $unityProject 'Assets\Rts\ScriptableObjects\MotionProfiles') -Filter '*.asset.meta' -ErrorAction SilentlyContinue |
    ForEach-Object { Remove-TrailingWhitespace -Path $_.FullName }

Write-Host "Stage 6 Unity validation passed. Logs: $createLog ; $validateLog ; $smokeLog"
