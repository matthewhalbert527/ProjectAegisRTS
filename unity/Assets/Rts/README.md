...
- `Scripts/Rendering`: board, actor, selection, low-power, production, interpolation, movement profile, vehicle, infantry, aircraft, turret, path-preview, building animation, combat visuals, economy visuals, visibility/fog/minimap visuals, AI intent/timeline visuals, map terrain/path debug visuals, and Stage32 terrain-piece set-dressing visuals.
- `Scripts/Rendering/TerrainPieces`: Unity-side terrain-piece definitions, catalogs, set-dressing profiles, runtime resolver/renderer, review controller, and QA HUD.
...
Stage 32 keeps gameplay routing unchanged while improving battlefield presentation. `TerrainPieceDefinition`, `TerrainPieceLibrary`, `TerrainSetDressingProfile`, and `TerrainSetDressingRuntimeLayer` are Unity-only visual metadata and rendering helpers. They instantiate collider-free prefabs under `BoardRoot` and never write terrain, passability, placement, resource, actor, fog, AI, or combat state back into `Rts.Core`.
